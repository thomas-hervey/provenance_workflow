...
with sqlite3.connect(DATABASE_PATH) as connection:
    ...
    c.executemany("""INSERT INTO users (username, email, password, role)
                      VALUES (?, ?, ?, ?)""", data)
    ...
    # insert data to users table
    c.executemany("""INSERT INTO tasks (name, due_date, priority, posted_date, status)
                      VALUES (?, ?, ?, ?, ?)""", data)
    ...
    # temporarily change the name of searches table
    c.execute("""ALTER TABLE searches RENAME TO old_searches""")

    # recreate a new users table with updated schema
    db.create_all()

    # retrieve data from old_tasks table
    c.execute("""SELECT name, source, data, posted_date, user_id
                         FROM old_searches
    ...
    # insert data to searches table
    c.executemany("""INSERT INTO searches (name, source, data, posted_date, user_id)
                          VALUES (?, ?, ?, ?, ?)""", data)

    # delete old_users table
    c.execute("DROP TABLE old_searches")
